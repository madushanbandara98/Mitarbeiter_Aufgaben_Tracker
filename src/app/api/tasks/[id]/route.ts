...
export async function PUT(request: Request, context: Context) {
  ...
  const task = await updateDb((db) => {
    ...
    const existing = db.tasks.find((entry) => entry.id === Number(id));
    if (!existing) {
      return undefined;
    }

    if (currentUser.userType !== "admin" && existing.employeeEmail !== currentUser.email) {
      return undefined;
    }
    ...
    const owner = currentUser.userType === "admin" && validated.data.employeeEmail
      ...
      : db.users.find((entry) => entry.email === existing.employeeEmail) ?? currentUser;
  ...
  if (task === undefined) {
    return NextResponse.json({ message: "Aufgabe nicht gefunden." }, { status: 404 });
  }
...
export async function DELETE(_request: Request, context: Context) {
  ...
  const removed = await updateDb((db) => {
    ...
    db.tasks = db.tasks.filter((entry) => {
      ...
      return entry.employeeEmail !== currentUser.email;
    });
